...
function validateForm(startDate, endDate, name, description, nodeJs, nextJs, reactJs, python, imageFile) {
    // validasi input nama project
    if (name == '') {
        alert('Project Name must be filled in')
        return false
    }

    // validasi input start date & end date
    if (startDate == "" || endDate == "") {
        alert('Start and End Date must be filled in')
        return false
    }

    // validasi input description
    if (description == '') {
        alert('Description must be filled in')
        return false
    }

    // klo tidak ada satupun tech yg di check maka tampilkan alert
    if (nodeJs == false && nextJs == false && reactJs == false && python == false) {
        alert('Atleast 1 technolgy should be checked')
        return false
    }

    if (!imageFile[0]) {
        alert('Image mush be uploaded')
        return false
    }
...
function submitBlog() {
    ...
    const validationResult = validateForm(startDateValue, endDateValue, inputName, inputDescription, nodeJs, nextJs, reactJs, python, inputImage)
